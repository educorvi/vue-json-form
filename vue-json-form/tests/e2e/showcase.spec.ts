...
test.describe('Structure', () => {
  async function checkRadios(page, containerID: string, values: string[]) {
    ...
    for (let index = 0; index < values.length; index++) {
      const radio = container.locator('> *').nth(index).locator('input').first();
  ...
  test('Title', async ({ page }) => {
    ...
    await expect(select.locator('option').nth(0)).toHaveValue('Mrs.');
    await expect(select.locator('option').nth(1)).toHaveValue('Mr.');
    await expect(select.locator('option').nth(2)).toHaveValue('Ms.');
    await expect(select.locator('option').nth(3)).toHaveValue('Dr.');
  });

  test('Fanciness', async ({ page }) => {
    const id = 'div#vjf_control_for__properties_fanciness';
    await expect(page.locator(id)).toBeVisible();
    await checkRadios(page, id, ['fancy', 'fancier', 'fanciest', 'unicorn']);
  });
  ...
  test.describe('Group', () => {
    test('dueDate', async ({ page }) => {
      ...
      await expect(page.locator(id)).toBeVisible();
      await expect(page.locator(id)).toHaveAttribute('type', 'datetime-local');
    });
    ...
    test('Description', async ({ page }) => {
      ...
      await expect(description).toHaveValue('This good text was set as default');
    ...
    test('Weekdays', async ({ page }) => {
      ...
      for (let index = 0; index < weekdays.length; index++) {
        const checkbox = container.locator('> *').nth(index).locator('input').first();
    ...
    test('RecurrenceInterval', async ({ page }) => {
      ...
      const sibling = await input.locator('xpath=following-sibling::*[1]');
    ...
    test('TestArray', async ({ page }) => {
      ...
      const container = page.locator(id);
      await expect(container).toBeVisible();
      await expect(container.locator('input')).toBeVisible();
      await expect(container.locator('input')).toHaveAttribute('placeholder', 'This is a placeholder');
      await expect(container.locator('button[aria-label="Add Item"]')).toBeVisible();
    });
  });

  test.describe('Object', () => {
    test.beforeEach(async ({ page }) => {
      await page.locator('#vjf_control_for__properties_group_selector').locator('> *').nth(3).click();
    });

    test('name', async ({ page }) => {
      const id = 'input#vjf_control_for__properties_testObject_properties_petName';
      await expect(page.locator(id)).toBeVisible();
      await expect(page.locator(id)).toHaveAttribute('type', 'text');
    });

    test('age', async ({ page }) => {
      const id = 'input#vjf_control_for__properties_testObject_properties_age';
    ...
    test('flauschig', async ({ page }) => {
      const id = 'input#vjf_control_for__properties_testObject_properties_flauschig';
      await expect(page.locator(id)).toBeVisible();
  ...
  test('HTML text', async ({ page }) => {
    ...
    await expect(page.locator(id)).toBeVisible();
    await expect(page.locator(id)).toHaveHTML('Ich bin ein <strong class="text-primary">HTML</strong> Text');
  });
  ...
  test('Fancy Unicorn', async ({ page }) => {
    await page.locator('#vjf_control_for__properties_fanciness input').nth(3).check();
    await page.locator('input[type=range]#vjf_control_for__properties_rating').fill('4');
    
    const id = 'span.vjf_htmlRenderer > h3';
    await expect(page.locator(id)).toHaveHTML('You are a very fancy unicorn my friend...');
...
test.describe('Button functions', () => {
  ...
  test('submit with missing fields', async ({ page }) => {
    await page.locator('#vjf_control_for__properties_name').fill('Test User');
    await page.locator('button[type="submit"]:not([formnovalidate])').click();
  ...
  test('reset form', async ({ page }) => {
    ...
    await expect(page.locator('#vjf_control_for__properties_name')).toHaveValue('');
  });

  test('submit with data (group)', async ({ page }) => {
    await page.locator('#vjf_control_for__properties_done').check();
    await page.locator('#vjf_control_for__properties_title').selectOption('Mr.');
    await page.locator('#vjf_control_for__properties_name').fill('Test User 2');
    await page.locator('#vjf_control_for__properties_fanciness input').nth(3).check();
    await page.locator('#vjf_control_for__properties_due_date').fill('2000-12-11T14:24');
    await page.locator('#vjf_control_for__properties_description').click();
    await page.locator('#vjf_control_for__properties_teststring').fill('test');
    await page.locator('#vjf_control_for__properties_weekdays input').nth(0).check();
    await page.locator('#vjf_control_for__properties_weekdays input').nth(4).check();
    await page.locator('#vjf_control_for__properties_recurrence_interval').fill('4');
    await page.locator('#vjf_control_for__properties_testArray input').nth(0).fill('Hello');
    await page.locator('#vjf_control_for__properties_testArray input').nth(1).fill('World');
    await page.locator('#vjf_control_for__properties_testArray > .btn-outline-primary').click();
    await page.locator('#vjf_control_for__properties_testArray input').nth(2).fill('I am third!');
    ...
    expect(json['description']).toEqual('This good text was set as default');
  ...
  test('submit with data (object)', async ({ page }) => {
    ...
    await page.locator('#vjf_control_for__properties_group_selector label').nth(1).click();

    await page.locator('#vjf_control_for__properties_testObject_properties_petName').fill('Richie');
    await page.locator('#vjf_control_for__properties_testObject_properties_age').clear();
    await page.locator('#vjf_control_for__properties_testObject_properties_age').fill('15');
    await page.locator('#vjf_control_for__properties_testObject_properties_flauschig').check();
