...
const expectSelectOptions = async (page, selector: string, expectedValues: string[]) => {
  ...
  for (let index = 0; index < expectedValues.length; index++) {
    await expect(options.nth(index)).toHaveAttribute('value', expectedValues[index]);
  }
};

test('JSO-43', async ({ page }) => {
  const BOOL_FIELD = '#vjf_control_for__properties_jso-43_properties_bool';
  const HALLO_FIELD = '#vjf_control_for__properties_jso-43_properties_hallo input[value="du"]';
  const HALLO_FIELD_ICH = '#vjf_control_for__properties_jso-43_properties_hallo input[value="ich"]';
  const REQUIRED_FIELD = '#vjf_control_for__properties_jso-43_properties_abhaengiges-feld';
  const REQUIRED_LABEL = 'label[for="vjf_control_for__properties_jso-43_properties_abhaengiges-feld"]';
...
test('JSO-79 (I)', async ({ page }) => {
  ...
  const SECOND_SELECT = '#vjf_control_for__properties_jso-79_properties_second';
...
test('JSO-79-IV', async ({ page }) => {
  ...
  await expect(page.locator('#vjf_control_for__properties_string-dep-required-2')).not.toHaveAttribute('required', '');
  await expect(page.locator('label[for="vjf_control_for__properties_string-dep-required-2"] span')).not.toContainText('*');

  await page.locator('#vjf_control_for__properties_string-dep-required').fill('Test');
  await expect(page.locator('#vjf_control_for__properties_string-dep-required-2')).toHaveAttribute('required', '');
  await expect(page.locator('label[for="vjf_control_for__properties_string-dep-required-2"] span')).toContainText('*');

  await page.locator('#vjf_control_for__properties_string-dep-required').clear();
  await expect(page.locator('#vjf_control_for__properties_string-dep-required-2')).not.toHaveAttribute('required', '');
  await expect(page.locator('label[for="vjf_control_for__properties_string-dep-required-2"] span')).not.toContainText('*');
});
...
test('JSO-68', async ({ page }) => {
  ...
  await expect(page.locator(`${ARRAY_CONTAINER} button[aria-label="Add Item"]`)).toBeVisible();
  ...
  const parent = page.locator(SINGLE_UPLOAD).locator('xpath=ancestor::div[@class="vjf_control"]');
...
test('Pattern string', async ({ page }) => {
  ...
  await page.locator('input[name="/properties/patternString"]').fill('mystring-abc');
  await submitForm(page);
...
test('Edit Symbols', async ({ page }) => {
  ...
  const parent = page.locator('input[name="/properties/obj-l/properties/auswahlfeld"]')
...
test('JSO-58', async ({ page }) => {
  ...
  const helpText = page.locator('input[name="/properties/bool-mit-hilfe"]')
...
test('JSO-51', async ({ page }) => {
  ...
  await expect(page.locator('div[name="/properties/jso-51-arr"] .vjf_htmlRenderer').nth(0))
    .toHaveText('Pre html');
  await expect(page.locator('div[name="/properties/jso-51-arr"] .vjf_htmlRenderer').nth(1))
...
test('JSO-44', async ({ page }) => {
  ...
  await expect(page.locator('div[name="/properties/abhaengiges-array"]')).not.toBeVisible();
  await page.locator('input[name="/properties/auswahlfeld"]').check();
  await expect(page.locator('div[name="/properties/abhaengiges-array"]')).toBeVisible();
  await expect(page.locator('div[name="/properties/abhaengiges-array"] input[type="text"]')).not.toBeVisible();
  await page.locator('div[name="/properties/abhaengiges-array"] input[type="checkbox"]').check();
  await expect(page.locator('div[name="/properties/abhaengiges-array"] input[type="text"]')).toBeVisible();
});

test('JSO-37', async ({ page }) => {
  await page.goto('http://localhost:5173/reproduce?nonav=true');
  
  await expect(page.locator('input[name="/properties/jso-37-field1"]')).toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field2"]')).not.toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field3"]')).not.toBeVisible();

  await page.locator('input[name="/properties/jso-37-field1"]').fill('a');

  await expect(page.locator('input[name="/properties/jso-37-field1"]')).toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field2"]')).toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field3"]')).not.toBeVisible();
  ...
  await expect(page.locator('input[name="/properties/jso-37-field1"]')).toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field2"]')).toBeVisible();
  await expect(page.locator('input[name="/properties/jso-37-field3"]')).toBeVisible();
...
test('JSO-34', async ({ page }) => {
  ...
  const containerId = '#vjf_control_for__properties_array141d1d356e40b4062ac2b1476ff52648c';
  
  await page.locator(`${containerId} > button`).click();
  await page.locator(`${containerId} > button`).click();
  await page.locator(`${containerId} > .list-group > div:first-child button.btn-outline-primary`).click();
  await page.locator(`${containerId} > .list-group > div:first-child button.btn-outline-primary`).click();
  await page.locator(`${containerId} > .list-group > div:nth-child(2) button.btn-outline-primary`).click();
  await page.locator(`${containerId} > .list-group > div:nth-child(2) button.btn-outline-primary`).click();
  
  await expect(page.locator(`${containerId} input[type="text"]`)).not.toBeVisible();

  await page.locator(`${containerId} > .list-group > div:first-child .list-group > div:first-child input[type="checkbox"]`).check();
  await expect(page.locator(`${containerId} > .list-group > div:first-child .list-group > div:first-child input[type="text"]`)).toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:first-child .list-group > div:nth-child(2) input[type="text"]`)).not.toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:nth-child(2) .list-group > div:nth-child(1) input[type="text"]`)).not.toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:nth-child(2) .list-group > div:nth-child(2) input[type="text"]`)).not.toBeVisible();

  await page.locator(`${containerId} > .list-group > div:nth-child(2) .list-group > div:nth-child(2) input[type="checkbox"]`).check();
  await expect(page.locator(`${containerId} > .list-group > div:first-child .list-group > div:first-child input[type="text"]`)).toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:first-child .list-group > div:nth-child(2) input[type="text"]`)).not.toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:nth-child(2) .list-group > div:nth-child(1) input[type="text"]`)).not.toBeVisible();
  await expect(page.locator(`${containerId} > .list-group > div:nth-child(2) .list-group > div:nth-child(2) input[type="text"]`)).toBeVisible();
});
...
test('JSO-23 & JSO-24', async ({ page }) => {
  ...
  await expect(page.locator('.vjf_showOnWrapper button[type="submit"].btn-primary')).toBeVisible();
  await expect(page.locator('.vjf_showOnWrapper button[type="reset"].btn-danger')).toBeVisible();

  const arrayId = 'div[name="/properties/array-11d6266e4c57d4363b1f4ef6d72025f3a"]';
  
  await page.locator(`${arrayId} button[aria-label="Add Item"]`).click();
  await page.locator(`${arrayId} button[aria-label="Add Item"]`).click();
  await expect(page.locator(`${arrayId} input[type="text"]`)).not.toBeVisible();
  await page.locator(`${arrayId} .list-group > div:nth-child(2) input[type="checkbox"]`).check();
  await expect(page.locator(`${arrayId} input[type="text"]`)).not.toBeVisible();
  await page.locator(`${arrayId} .list-group > div:nth-child(1) input[type="checkbox"]`).check();
  await expect(page.locator(`${arrayId} .list-group > div:nth-child(1) input[type="text"]`)).toBeVisible();
  await expect(page.locator(`${arrayId} .list-group > div:nth-child(2) input[type="text"]`)).toBeVisible();
});

test('JSO-11', async ({ page }) => {
  await page.goto('http://localhost:5173/reproduce?nonav=true');
  
  await expect(page.locator('input[name="/properties/upload-field-in-formcd69370e0708472482997b3da12ad3cc"]')).not.toBeVisible();
  await page.locator('input[name="/properties/textline-in-form736e3a96a17d436996e5c8489cb9d102"]').fill('abc');
  await expect(page.locator('input[name="/properties/upload-field-in-formcd69370e0708472482997b3da12ad3cc"]')).toBeVisible();
  
  await page.reload();
  
  await expect(page.locator('input[name="/properties/upload-field-in-formcd69370e0708472482997b3da12ad3cc"]')).not.toBeVisible();
  await page.locator('input[name="/properties/selectionfield-in-formad0995330a9343efbb2e5488ab28e4a8"][value="option 11"]').check();
  await expect(page.locator('input[name="/properties/upload-field-in-formcd69370e0708472482997b3da12ad3cc"]')).toBeVisible();
});
...
test('JSO-7', async ({ page }) => {
  ...
  await page.locator('input[name="/properties/email"]').fill('test@example.com');
  await submitForm(page);
