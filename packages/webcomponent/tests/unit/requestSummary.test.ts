...
describe('AiMockServer – CORS', () => {
    it('GET /ai/prompt-types includes CORS headers', async () => {
        const res = await fetch(`${baseUrl}/ai/prompt-types`);
        expect(res.headers.get('access-control-allow-origin')).toBe('*');
    });

    it('POST /ai/summary SSE response includes CORS headers', async () => {
        const formData = new FormData();
        formData.append('promptType', 'claim-summary');
        formData.append(
            'document',
    ...
    it('OPTIONS preflight returns 204 with CORS headers', async () => {
        const res = await fetch(`${baseUrl}/ai/summary`, { method: 'OPTIONS' });
    ...
    it('error responses also include CORS headers', async () => {
        const errServer = new AiMockServer({
            summaryHttpError: { status: 500, message: 'oops' },
        });
        const url = await errServer.start();
        const formData = new FormData();
        formData.append('promptType', 'x');
        formData.append('document', new Blob(['%PDF']));
