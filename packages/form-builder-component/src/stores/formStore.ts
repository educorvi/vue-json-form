...
export interface FormStore {
    ...
    updateControlSchemaProperty: (controlId: string, updates: Partial<JSONSchema>) => void;
    ...
    renameControlKey: (controlId: string, oldKey: string, newKey: string) => void;
    ...
    setRootToLayout: (type?: 'VerticalLayout' | 'HorizontalLayout' | 'Group') => void;
    addWizardPage: () => void;
...
function setupFormStore(): FormStore {
        ...
        const uiSchema = computed<UISchema>(() => ({
            ...
            ).toUiSchema() as LayoutElement | WizardElement,
        }));
        ...
        const exportedJsonSchemaIsEmpty = computed<boolean>(() => {
            return (
                Object.keys(
                    (exportedJsonSchema.value.properties as object) ?? {}
                ).length === 0
            );
        });
        ...
        function migrateSchemaToRoot(schemaKey: string, fromParentId: string): void {
            const parentEl = findElementById(
                getAllRootElements(),
                fromParentId
            );
            if (!parentEl) return;
