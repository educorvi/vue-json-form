...
```bash
yarn add @educorvi/vue-json-form-builder
```
...
This is exactly how `apps/json-forms-builder` embeds the builder (`app/pages/forms/detail.vue`): the user is already logged in via `nuxt-auth-utils`, so only the collab URL is configured — the browser's session cookie authenticates the WebSocket handshake automatically.
