...
const containerElementDefaults = {
    children: [],
    layout: Layout.Vertical,
    showTitle: true,
};
type ContainerElementOptionalKeys =
    keyof typeof containerElementDefaults | BaseDataElementOptionalKeys;
    ...
    toJsonSchema(generator: SchemaGenerator, scope: string[]): JSONSchema {
        ...
        if (this.children && this.children.length > 0) {
            const { childrenJsonSchema, requiredList } =
                generator.generateJsonSchemaForElements(
                    this.children,
                    scope.concat(this.getScopePart())
                );
...
type ArrayElementOptionalKeys =
    keyof typeof arrayElementDefaults | ContainerElementOptionalKeys;
export class ArrayElement extends ContainerElement {
    ...
    constructor(
        data: Omit<
            PartialBy<ArrayElementData, ArrayElementOptionalKeys>,
            'type'
        >
    ) {
        super(data);
    ...
    protected static setDefaults(
        data: PartialBy<ArrayElementData, ArrayElementOptionalKeys>
    ): ArrayElementData {
        return {
            ...super.setDefaults(data),
    ...
    static fromJsonSchemaAndUiSchema(
        ...
        const layout = uiSchema.options?.uiSchema?.type
            ? (uiSchema.options.uiSchema.type as Layout)
            : Layout.Vertical;
...
type ObjectElementOptionalKeys =
    keyof typeof objectElementDefaults | ContainerElementOptionalKeys;
export class ObjectElement extends ContainerElement {
    ...
    constructor(
        data: Omit<
            PartialBy<ObjectElementData, ObjectElementOptionalKeys>,
            'type'
        >
    ) {
        super(data);
    ...
    protected static setDefaults(
        data: PartialBy<ObjectElementData, ObjectElementOptionalKeys>
    ): ObjectElementData {
        return {
            ...super.setDefaults(data),
