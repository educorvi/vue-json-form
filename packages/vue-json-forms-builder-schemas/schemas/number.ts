...
export class NumberElement extends SimpleElement {
    ...
    static schema = SimpleElement.schema
        ...
        .superRefine((data, ctx) => {
            ...
            if (
                ...
                (data.minimum === undefined ||
                    data.maximum === undefined ||
                    data.multipleOf === undefined)
            ) {
                ctx.addIssue({
                    code: 'custom',
                    message:
                        'range requires minimum, maximum, and multipleOf to be defined',
                    value: data,
                });
            }
        });
    ...
    constructor(
        data: Omit<
            PartialBy<NumberElementData, NumberElementOptionalKeys>,
            'type'
        >
    ) {
        super(data);
    ...
    protected static setDefaults(
        data: PartialBy<NumberElementData, NumberElementOptionalKeys>
    ): NumberElementData {
        return {
    ...
    static fromJsonSchemaAndUiSchema(
        ...
        if (
            ...
            Object.values(NumberFormat as unknown as string[]).includes(
                jsonSchema.type
            )
        ) {
            ...
            throw new Error(
                'Invalid type for NumberElement: ' + jsonSchema.type
            );
        }
