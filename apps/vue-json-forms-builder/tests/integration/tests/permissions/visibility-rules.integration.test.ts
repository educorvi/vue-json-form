...
describe('Visibility rules — private groups require private children', () => {
    ...
    it('rejects making a group private when a descendant (sub-sub-group) is visible', async () => {
        // Given: group → sub-group (visible) → sub-sub-group (visible).
        // Note: a `visible` node under a `private` parent is impossible —
        // creation coerces children of private parents to `private`, so
        // the chain must be visible all the way down to set up a deep
        // visible descendant.
        const sub = await admin.client.groups.create({
            body: { title: 'Sub', name: 'sub', visibility: 'visible' },
            query: { parent: String(groupId) },
        });
