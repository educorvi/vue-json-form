...
export class PermissionService {
    ...
    async getResolvedGroupPermissions(
        ...
        // TypeORM's materialized-path findAncestors includes the target
        // itself — the inherited chain is everything ABOVE the group.
        const inheritedChain = ancestors.filter((a) => a.id !== groupId);
        const ancestorIds = [
            ...new Set([...inheritedChain.map((a) => a.id), groupId]),
        ];
        ...
        const directUserIds: string[] = [
            ...new Set(
                (rows as Array<{ group_id: number; user_id: string | null }>)
                    .filter((r) => Number(r.group_id) === groupId)
                    .map((r) => r.user_id)
        ...
        const inheritedRoles = await this._fetchHighestInheritedRoles(
            ...
            'group',
            groupId,
            inheritedChain.map((a) => a.id)
        );

        // Build ancestor map for source group resolution
        const ancestorMap = new Map(inheritedChain.map((a) => [a.id, a]));
        const now = new Date();
    ...
    private async _resolveAncestorIds(
        ...
        if (scopeKey === 'group') {
            ...
            const ancestors = await treeRepo.findAncestors(target);
            // TypeORM's materialized-path findAncestors INCLUDES the target
            // itself — the chain is everything ABOVE the group.
            return ancestors
                .filter((a) => a.id !== scopeValue)
                .map((a) => a.id);
        }
        const formRepo = this.repo.manager.getRepository(Form);
        const form = await formRepo.findOne({
