...
2. Type the **numeric id** of a form of the builder backend into the input field (as shown in the main app's form list) and press Enter. The webcomponent mounts and runs keycloak-js against kc1 (`http://localhost:8080`, realm `dev`, public client `vueformbuilder-embed`):
...
1. The builder connects to the collab websocket (`ws://localhost:1234`, document name = form id) with the kc1 access token as `Authorization: Bearer` (validated by the backend via JWKS, `NUXT_AUTH_ALLOWED_CLIENT_IDS`) and Hocuspocus hydrates the form from its stored definition.
1. Loading another form id re-mounts the builder with the new document.
2. *Logout* clears the local external session (the kc1/external Keycloak sessions stay intact).
